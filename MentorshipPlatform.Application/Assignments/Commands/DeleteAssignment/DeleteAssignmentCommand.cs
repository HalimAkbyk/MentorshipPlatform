...
namespace MentorshipPlatform.Application.Assignments.Commands.DeleteAssignment;
...
public class DeleteAssignmentCommandHandler : IRequestHandler<DeleteAssignmentCommand, Result<bool>>
{
    ...
    public async Task<Result<bool>> Handle(DeleteAssignmentCommand request, CancellationToken cancellationToken)
    {
        ...
        if (assignment.MentorUserId != _currentUser.UserId.Value)
            return Result<bool>.Failure("Bu odevi silme yetkiniz yok");

        if (assignment.Status == AssignmentStatus.Draft)
        {
            // Hard delete if Draft
            _context.Assignments.Remove(assignment);
        }
        else
        {
            // Soft delete (close) if Published
            assignment.Close();
        }
