...
namespace MentorshipPlatform.Application.Courses.Commands.ResubmitForReview;

public record ResubmitCourseForReviewCommand(Guid CourseId, string? MentorNotes) : IRequest<Result>;

public class ResubmitCourseForReviewCommandValidator : AbstractValidator<ResubmitCourseForReviewCommand>
{
    public ResubmitCourseForReviewCommandValidator()
    {
        RuleFor(x => x.CourseId).NotEmpty();
    }
}

public class ResubmitCourseForReviewCommandHandler : IRequestHandler<ResubmitCourseForReviewCommand, Result>
{
    ...
    public async Task<Result> Handle(ResubmitCourseForReviewCommand request, CancellationToken cancellationToken)
    {
        ...
        // Status MUST be RevisionRequested only
        if (course.Status != CourseStatus.RevisionRequested)
            return Result.Failure("Kurs yalnızca Revizyon İstendi durumunda tekrar gönderilebilir");
        ...
        // Check if any flagged videos were changed
