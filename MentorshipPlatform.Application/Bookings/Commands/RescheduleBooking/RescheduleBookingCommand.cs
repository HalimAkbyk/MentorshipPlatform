...
public class RescheduleBookingCommandHandler : IRequestHandler<RescheduleBookingCommand, Result>
{
    ...
    public async Task<Result> Handle(RescheduleBookingCommand request, CancellationToken cancellationToken)
    {
        ...
        if (isStudent)
        {
            ...
            // Notify mentor about the reschedule
            try
            {
                await _emailService.SendTemplatedEmailAsync(
                    EmailTemplateKeys.RescheduleRequest,
                    booking.Mentor.Email!,
                    new Dictionary<string, string>
                    {
                        ["otherPartyName"] = booking.Student.DisplayName,
                        ["oldDate"] = oldStartAt.ToString("dd MMMM yyyy HH:mm", trCulture),
            ...
            {
                _logger.LogWarning(ex, "Failed to send reschedule email to mentor for {BookingId}", booking.Id);
            }
        ...
        else
        {
            ...
            {
                _logger.LogWarning(ex, "Failed to send reschedule request email to student for {BookingId}", booking.Id);
            }
